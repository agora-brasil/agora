(function () {
  ...
  .controller('TopicsShow', ['$scope', '$stateParams', 'Topic', 'User', 'collapseBody',
    function ($scope, $stateParams, Topic, User, collapseBody) {
      function getProposalGroup(topic) {
      ...
      $scope.joinProposalGroup = function () {
        $scope.topic.addCurrentUserToProposalGroup();
        getProposalGroup($scope.topic);
      };
    }
  ])
